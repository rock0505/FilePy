...
async def upload_file(parent_id: int = Form(1), file: UploadFile = File(...), current=Depends(get_current_user)):
    ...
    class ReadWrapper:
        ...
        async def read(self): return await self.f.read()
...
async def download(file_id: int, current=Depends(get_current_user)):
    ...
    if not check_perm(current['id'], file_id, 'r'):
        raise HTTPException(status_code=403, detail='No read permission')
    path, compressed, size = get_blob_path(f['blob_id'])
    # stream file
    def iterfile():
        if compressed:
    ...
    return StreamingResponse(iterfile(), media_type='application/octet-stream', headers={'Content-Disposition': f'attachment; filename="{f["name"]}"'})
